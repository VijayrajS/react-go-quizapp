...
class Admin_quiz extends Component {
  constructor() {
    super();
    this.state = {
      data: [],
  ...
  Submit (event) {
    ...
    fetch('http://127.0.0.1:8080/quizzes/'+this.state.value, {
     method: 'DELETE',
   })
      .then(response => {
  ...
  componentDidMount() {
    fetch('http://127.0.0.1:8080/quizzes/',{
      method:'GET'
    })
          ...
          <table className="table-hover">
            ...
            <tbody>{this.state.data.map((item, key)=>{
                 return (
                    <tr key = {key}>
                        <td>{item.id}</td>
                        <td><a href={'/Admin/quizedit/'+item.id}>{item.name}</a></td>
